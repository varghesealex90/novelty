...
cmd:option('-epochs',2,'epochs: number of epochs')
...
if assert(type(testData) == "userdata") then
	testData = {
	data = testData;
	}
	...
	if opt.size == 'small' then
		testData.data = testData.data[{{1,100}}]
		testData.labels = testData.labels[{{1,100}}]
	end
end
